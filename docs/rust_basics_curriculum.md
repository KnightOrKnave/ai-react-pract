...
### チャプター別の講義資料と練習問題（配布用）

**チャプター1: Rustの第一歩**
- 資料: `cargo new/run/check`の最低限ワークフロー図、`println!`のフォーマット指定子一覧 (`{}`, `{:?}`, `{name}`)、デバッグビルドとリリースビルドの出力場所比較 (`target/debug` vs `target/release`)。
- 練習問題:
  1. `cargo run --release`と`cargo run`の実行時間を簡易計測し、差分をコメントでまとめる（`time`コマンドでOK）。
  2. `println!`で左寄せ・右寄せ・ゼロ埋めを試すミニプログラムを書く。
  3. `cargo check`で通り、`cargo run`で失敗するケースを1つ作り、理由をコメントで整理する。

**チャプター2: 変数・型・所有権の直感**
- 資料: スタック/ヒープの図解、`Copy`トレイトが自動付与される型の表、`String`のムーブ・クローンのメモリ挙動。
- 練習問題:
  1. `Copy`かどうかを`std::mem::size_of`と`println!`で観察し、`Copy`でない型のムーブ後に起きるコンパイルエラーを確認する。
  2. `String`を引数に受けて返さない関数と、`&str`を引数にする関数を用意し、呼び出し元での使用可否を表にまとめる。
  3. `drop`を明示的に呼ぶ例と暗黙のスコープ終了の例を作り、実行順序を`println!`で追跡する。

**チャプター3: 制御フローとマッチング思考**
- 資料: `match`の網羅性チェック例、パターンとガードの組み立て方、`loop { break value; }`で値を返す図解。
- 練習問題:
  1. FizzBuzzを`match`ベースで実装し、`if`版との可読性を比較するコメントを書く。
  2. `while let Some(x)`で`Vec`を後ろから取り出す例を作り、`pop`との違いを説明する。
  3. `match`で`Option<Result<T, E>>`を分解する小さな関数を書き、パターンの読みやすさを確認する。

**チャプター4: 所有権・借用・ライフタイムの基礎**
- 資料: 可変借用と不変借用の同時性ルール、ライフタイム省略規則の3パターン、`&[T]`と`&str`の境界チェック挙動。
- 練習問題:
  1. 可変借用を2回同時に行うコードをあえて書き、エラーメッセージを抜粋して和訳する。
  2. `fn first_char(s: &str) -> Option<char>`を実装し、借用を返すケースと所有権を返すケースの違いを説明する。
  3. `fn longest<'a>(a: &'a str, b: &'a str) -> &'a str`を実装し、ライフタイム注釈を外すとどうなるかを確認する。

**チャプター5: コレクションとイテレーション**
- 資料: `Vec`/`HashMap`の所有権ルール（`insert`/`get`/`entry`）、`iter`/`iter_mut`/`into_iter`の対比表、`collect`の型注釈例。
- 練習問題:
  1. `HashMap<String, usize>`で単語頻度を計算し、`into_iter().collect::<Vec<_>>()`で並べ替えて上位3件を表示する。
  2. `Iterator`チェーンのみで`[1, 2, 3, 4, 5]`を2乗して合計するコードを書く（ループ禁止）。
  3. `VecDeque`を使い、キューとスタック両方の操作を`push_front/push_back/pop_front/pop_back`で試す。

**チャプター6: エラー処理とResult**
- 資料: `Result`と`Option`の使い分けフロー、`?`演算子のデシュガー、`thiserror`でのカスタムエラー定義サンプル。
- 練習問題:
  1. `fn read_number(path: &str) -> Result<i64, Box<dyn Error>>`を実装し、`?`を用いてIOエラーを伝搬する。
  2. `Option`を`Result`に変換するユーティリティを2通り（`ok_or`と`ok_or_else`）で書き、挙動を比較する。
  3. `anyhow`を導入し、同じ関数を`anyhow::Result`で書き換えたときのコード量とトレードオフをコメントする。

**チャプター7: モジュール・テスト・Cargo活用**
- 資料: ファイル配置と`mod`/`pub`の対応表、`cargo test`のフィルタリング（`cargo test name`）、`cfg(test)`の意味と使い方。
- 練習問題:
  1. `src/lib.rs`をサブモジュール2つに分け、公開するAPIを`pub use`で再輸出する。
  2. `tests/`配下に結合テストを追加し、`cargo test -- --nocapture`で出力を観察する。
  3. `cargo fmt`と`cargo clippy -- -D warnings`をCI前提で実行し、警告が出た箇所を修正する。

**チャプター8: ジェネリクス・トレイト・ライフタイム応用**
- 資料: `impl Trait`とジェネリックパラメータの違い、`where`句の読み方、トレイトオブジェクトと静的ディスパッチの比較図。
- 練習問題:
  1. ジェネリックな`Pair<T>`に`swap`と`as_tuple`を実装し、`T: Clone`の制約を付ける/外す場合を試す。
  2. `trait Renderer { fn render(&self) -> String; }`を複数型で実装し、`Vec<Box<dyn Renderer>>`を`for`で描画する。
  3. 参照を保持する構造体に明示ライフタイムを付ける例を作り、`'static`な文字列リテラルで動かすケースとそうでないケースを比較する。

**チャプター9: 仕上げのミニプロジェクト**
- 資料: 仕様策定のテンプレート（目的/機能/CLIフラグ/入出力例）、モジュール分割の基本方針（`lib.rs`でドメイン、`main.rs`でI/O）、品質チェックルーチン（fmt/clippy/test）。
- 練習問題:
  1. テーマを1つ選び、`README`に目的・入力・出力・想定ユーザーを書き出す。
  2. `src/lib.rs`でドメイン関数を`pub`にし、`src/main.rs`は引数パースと表示のみの薄い実装に分離する。
  3. `cargo clippy -- -D warnings`と`cargo test`を通すまでブラッシュアップし、改善した点をメモする。
